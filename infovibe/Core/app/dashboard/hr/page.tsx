...
export default async function HRDashboardPage() {
  ...
  if (user.role !== "hr") redirect(getHomeRoute(user.role));
  ...
  if (data?.stats) {
    stats = [
      ...
      { label: "Active Employees", value: data.stats.activeEmployees ?? 0, accent: "bg-emerald-500/10 text-emerald-400" },
      { label: "Pending Leaves", value: data.stats.pendingLeaveRequests ?? 0, accent: "bg-amber-500/10 text-amber-400" },
      { label: "Departments", value: data.stats.departmentsCount ?? 0, accent: "bg-blue-500/10 text-blue-400" },
    ];
  }
  ...
  return (
    ...
    >
      ...
      <section className="rounded-2xl border border-slate-700 bg-slate-900/60 p-6 backdrop-blur-sm">
        <h2 className="mb-4 text-lg font-bold text-white">HR Actions</h2>
